...
// Local helpers (static week starting 22 Aug 2025)
function addDays(d: Date, n: number) {
  const x = new Date(d);
  x.setDate(x.getDate() + n);
  return x;
}
const T0 = new Date('2025-08-22T00:00:00'); // fixed demo start
const T1 = addDays(T0, 7);
...
export default function GanttWeek({
  ...
  // Timeline bounds: fixed 2025-08-22 → +7 days
  const t0 = useMemo(() => T0, []);
  const t1 = useMemo(() => T1, []);
  ...
  // Day labels
  const days = useMemo(() => {
    return Array.from({ length: 7 }).map((_, i) => addDays(t0, i));
  }, [t0]);

  // Last-30-days failures per vehicle (relative to t0)
  const last30ByVehicle = useMemo(() => {
    ...
    for (const f of failures as any[]) {
      const vid = String(f.vehicleId ?? f.asset_id ?? '');
      const ds = String(f.date ?? f.failure_date ?? '');
      if (!vid || !ds) continue;
      const dt = new Date(ds.replace('Z', ''));
      if (dt >= cutoff && dt <= t1) byV.set(vid, (byV.get(vid) ?? 0) + 1);
    }
    return byV;
  }, [t0, t1]);
  ...
  function clampInterval(startISO?: string, endISO?: string) {
    ...
    const s = new Date(String(startISO).replace('Z', ''));
    const e = new Date(String(endISO).replace('Z', ''));
